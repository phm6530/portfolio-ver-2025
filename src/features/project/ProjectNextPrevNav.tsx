...
const ProjectNextPrevNav = () => {
  ...
  return (
    <>
      {list &&
        list.map((item, key) => {
          const isPage = item.isPage === "prev" ? "이전 글" : "다음 글";

          return (
            <Wrap key={key}>
              <PrevnextNav>{isPage}</PrevnextNav>
              <PrevnextSummaryWrapper
                onClick={() => navigate(`/project/${item.project_key}`)}
              >
                <ThumbNailStyle img={`${IMG_URL}/${item.thumbnail}`} />

                <PrevnextSummary>
                  <PrevnextTitle>{item.title}</PrevnextTitle>
                  <PrevnextDescription>{item.description}</PrevnextDescription>
                </PrevnextSummary>
              </PrevnextSummaryWrapper>
            </Wrap>
          );
